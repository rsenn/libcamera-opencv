...
void
EventLoop::timeoutTriggered(int fd, short event, void* arg) {
  EventLoop* self = static_cast<EventLoop*>(arg);
  self->exit();
}

void
EventLoop::timeout(unsigned int sec) {
  struct event* ev;
  struct timeval tv;
...
void
EventLoop::callLater(const std::function<void()>& func) {
  {
